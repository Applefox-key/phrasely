...
export default function AppSwitcher() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="p-2 rounded-md hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-500 dark:text-gray-400"
        title="Switch app"
      >
...
function AppCard({ app, onSelect }: AppCardProps) {
  ...
  if (app.comingSoon) {
    return (
      <div className={`${base} border-2 border-dashed border-gray-200 dark:border-slate-600 text-gray-400 dark:text-gray-500 cursor-not-allowed`}>
  ...
  return (
    <a
      href={app.url!}
      className={`${base} border border-gray-200 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300`}
      onClick={onSelect}
    >
      <span className="text-2xl">{app.icon}</span>
      <span className="text-xs font-medium">{app.name}</span>
