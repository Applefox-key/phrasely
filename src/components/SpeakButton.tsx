    ...
    <div className={`inline-flex items-center gap-0.5 ${className}`}>
      {langs.map(({ code, label }) => (
        <button
          ...
          className={`text-[10px] px-1 py-0.5 rounded transition-colors leading-none ${
            lang === code
              ? "bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-300 font-semibold"
      ...
      <button
        ...
        className={`inline-flex items-center justify-center w-7 h-7 rounded-full transition-colors shrink-0 ${
          speaking
            ? "text-teal-600 dark:text-teal-300 bg-teal-100 dark:bg-teal-900/40"
            : "text-gray-400 hover:text-teal-500 dark:hover:text-teal-400 hover:bg-gray-100 dark:hover:bg-gray-700"
        }`}
      >
        {speaking ? (
