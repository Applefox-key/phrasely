...
export default function ExpressionRow({
  ...
  return (
    <div
      className={`flex items-center gap-3 px-3 py-2.5 border-b border-gray-100 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-750 transition-colors group ${
    ...
    >
      ...
      {/* Status badge */}
      <span className={`hidden sm:inline text-xs rounded-full px-2 py-0.5 font-medium ${STATUS_COLORS[expression.status] ?? ''}`}>
        {expression.status}
      </span>

      {/* inQueue icon */}
      {expression.inQueue && (
        <span title="In queue" className="text-teal-500 dark:text-teal-400 text-xs">▶</span>
      )}
      ...
      <select
        ...
        onChange={(e) => { e.stopPropagation(); onLabelChange(e.target.value ? Number(e.target.value) : null); }}
        onClick={(e) => e.stopPropagation()}
